...
describe("DateTime", function() {
	...
	it("matches the passed in Date when called with one argument", function() {
		...
		for (let i = 0; i < dates.length; i++) { 
			expect(DateTime(dates[i]).offset).toEqual(dates[i].getTime()); 	// 1. toThrow() assumes a function was passed to expect. 2. Creating a function inside a loop in the straigthforward way is not natural in JS.
		}
	});

	it("throws an error when called with a single non-Date argument", function(){
		let nonDates = [0, NaN, Infinity, "", "not a date", null, /regex/, {}, []];
		for (let i = 0; i < nonDates.length; i++) { 
...
// Testing for obth typical dates and extreme dates
let testDates = [ 
...
let expectedValues = {
	...
	offset: [-62135596800000, -61501568033000, -52031843266000, -6113414499000, -2197654532000, -2163155365000, 17174202000, 
						966822169000, 1222246136000, 1477583643000, 4478364070000, 253402258037000, 8640000000000000, -8640000000000000, -62300000000000]
};
